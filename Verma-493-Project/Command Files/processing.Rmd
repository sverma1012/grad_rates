---
title: "Graduating Midwest Liberal Arts Colleges"
author: "Sneha Verma"
output: word_document
date created: 4/8/2022
purpose: consists of all the steps involved in transforming importable data files into full cleaned and processed data files that will be used to generate analysis data.
---
...
To read the data please install 'readxl' package before running the below code cells:
...
NOTE: Before running the cell below, make sure that you change the working directory path in setwd() to match the path in your device.
...
NOTICE: The values in the datasets imported above are in percentages, i.e., they have been converted from 80% to 0.80.
...
NOTICE: The values in the above dataset remain as in the excel sheet because there was no percentage sign in the values.
This will need to be corrected while wrangling and cleaning the data.
...
Both the above datasets' values were converted from percentages to decimal values.
...
These values cannot be seen within the first 10 columns due to formatting issues which will be corrected in the cleaning phase.
...
It can be seen that the values of the graduation rates have been converted from percentages to decimals like for Carleton College and Grinnell College.
...
NOTICE: The values in the importable data is not in percentages which is reflected in the tibble.


## Wrangling and Cleaning

As seen while importing the datasets, there are a lot of issues in the resulting tibbles because of formatting issues. In this section, I will wrangle the data to transform the "raw" data into a more usable form.
...
Now I will separate the dataset into two separate sets; one detailing the graduation rates by race and the other by gender.
...
Rename the columns to the appropriate years.
...
Split the first dataset from Grinnell College into data for sex and race.
...
The categories of pell status have grammatical errors. The next code cell will fix that.
...
The datasets provided by Lawrence University are already split in three different datasets disaggregating graduation rates by sex, race, and financial status. Hence, I only have to wrnagle the data into a clean format.
...
In the data, there are missing values that have values of 'n/a.' This inconsistency can cause issues while appending, so I will replace them with the default missing value of R.
...
Missing values in this dataset are indicated by two asterisks. I will replace it with the standard NA values.
...
All of the data have been wrangled into formats that will allow for easy appending.


## Wrangling and Cleaning (Race)

In this section, I will be cleaning the individual race datasets.

First, I will be imputing the datasets for each category. Second, I will be cleaning the categories of race for each college to maintain consistency

No imputation will take place. This is to avoid biases since imputations of 0 will indicate that people from a certain category of a race did not graduate at all.

For each college, I will check that the categories of race are consistent.
...
Looking at the results above, the following categories of race are consistent among most colleges:
1) Asian
...
Now, I will append these datasets.

First, convert all rows to numeric (<dbl>)
...
Looking at the wrangled data from Macalester, it can be seen that it is in whole number values while the data from the rest of the colleges are in decimal format.
...
Now, pivot the dataset into long format to be able to conduct multiple linear regression.
...
The resultant tibble has categorized 'year' as a character. However, to apply year fixed effects, the variable will have to be converted into numeric format. 
...
## Wrangling and Cleaning (Sex)

In this section, I will be wrangling and clearning the individual datasets of graduation rates disaggregated by sex.

These datasets do not have any missing values, hence, imputation is not required.

```{r}
carleton_sex
```

All of the columns with graduation rates are of data type double, other than the column stating the sex. No wrangling is requried at this stage.

```{r}
grinnell_sex
```

Similarly, there is no imputation or wrangling required for this dataset.

```{r}
lawrence_sex
```

The case of the categories of sex for the Lawrence University dataset does not match the datasets of the previous two colleges.
...
All the datasets are in the same format. They can be appended now.

## Appending (Sex)

Now I will add a college column, as done for the data disaggregated by race.
...
The format of all three datasets are consistent. Now, they will be appended.
...
Now, I will pivot the dataset for analysis.
...
## Wrangling and Cleaning (Pell grant)

In this section, I will be wrangling and cleaning the graduation rate datasets disaggregated by pell grant recipients.

As observed while completing initial wrangling, none of these datasets have any missing values. Hence, no imputation is required.
...
Looking at the individual values for pell status, it can be seen that Coe College's third value does not match the third value of the other colleges'.
...
Due to discrepancies, I will make sure that the third category for pell status is the same across all colleges.
...
Now all the individual datasets match. Let's move on to appending.

## Appending (Pell grant)

Start with adding a college column for each individual set.
...
Now, the analysis and modeling of each dataset can take place!


## Export

For further summary and analysis, I will export the three datasets, so that they can be used in other scripts.
